...
export function Approach() {
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden border-y border-border/40 bg-surface/30 py-20 sm:py-28 md:py-32">
          ...
          {stages.map((stage, i) => (
            ...
            >
              <div className="relative z-10 flex h-16 w-16 items-center justify-center rounded-full border-2 border-border bg-background">
                <span className="font-mono text-lg font-bold text-primary">{stage.num}</span>
                <motion.div
                  initial={{ scale: 0 }}
                  whileInView={{ scale: 1 }}
                  viewport={{ once: true }}
